...
# --- Dashboard Title ---
st.markdown("<h1 style='text-align: center;'>📊 IB Transaction Ledger</h1>", unsafe_allow_html=True)
st.markdown("<div class='centered-subtitle'>November 2024 — April 2025</div>", unsafe_allow_html=True)

# --- Load CSV and Excel directly (assuming they exist in the same folder as the app)
csv_filename = "Combined_QR_Transfer_Reportupdate.csv"
excel_filename = "MERCHANT_TRANSACTION_REPORT_UPDATED.xlsx"

# --- Read CSV Data
df1 = pd.read_csv(csv_filename, skiprows=1, low_memory=False)

# --- Read Excel Data (optional)
df_excel = pd.read_excel(excel_filename)

# --- Preprocess CSV Data
df1 = df1.loc[:, ~df1.columns.str.contains('^Unnamed')]
df1.columns = df1.columns.map(str).str.strip().str.replace('\ufeff', '', regex=False)
