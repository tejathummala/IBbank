...
st.title("📊 Transaction Analysis(Past 6 months Transactions data)")

# File Path
folder_path = r"C:\Users\CGSPL\Desktop\IB Reports\IB Reports"
file_name = "MERCHANT TRANSACTION REPORT-FAIL REASON (1).xlsx"
...
if os.path.exists(file_path):
    ...
    with tab2:
            ...
            # Group by MEMBER_ID and count the unique merchants
            ...
            sns.barplot(data=merchant_grouping, x='MEMBER_ID', y='Unique_Merchants_Count', palette='viridis')
            plt.title('Unique Merchants Count per MEMBER_ID', fontsize=16)
            plt.xlabel('MEMBER_ID', fontsize=14)
            plt.ylabel('Unique Merchants Count', fontsize=14)
            ...
            sns.barplot(data=top_merchants, x='Transaction_Count', y='MERCHANT_NAME', palette='coolwarm')
            plt.title('Top 10 Merchants by Number of Transactions', fontsize=16)
            ...
            df_filtered = df[df['TXN_ID'] != 'FAILED_TXN']
                ...
                plt.title('Top Performing Merchants Based on Total Transaction Amount (Excluding Failed Transactions)')
                plt.ylabel('')  # Hide the y-label
                plt.show()
                st.subheader('🏆 Top Performing Merchants Based on Total Transaction Amount')
                st.pyplot(plt)

            plt.title("Top 5 Merchants by Transaction Count for Each Month")
                plt.xlabel("Month")
                plt.ylabel("Transaction Count")
                plt.legend(title="Merchant (Name & Mobile)", bbox_to_anchor=(1.05, 1), loc='upper left')
                plt.tight_layout()
                ...
                # Filter daily trend data for top merchants (or all merchants if needed)
                df['TXN_DATE'] = pd.to_datetime(df['TXN_DATE'], format='%d-%b-%y %H:%M:%S')
                df['TXN_DAY'] = df['TXN_DATE'].dt.date # for daily
                df['TXN_MONTH'] = df['TXN_DATE'].dt.to_period('M') # for monthly
                daily_trend = df.groupby(['MERCHANT_NAME', 'TXN_DAY']).size().reset_index(name='Transaction_Count')
                monthly_trend = df.groupby(['MERCHANT_NAME', 'TXN_MONTH']).size().reset_index(name='Transaction_Count')
                top_merchants = df['MERCHANT_NAME'].value_counts().head(5).index  # or use df['MERCHANT_NAME'].unique() for all
                filtered_trend = daily_trend[daily_trend['MERCHANT_NAME'].isin(top_merchants)]
                top_merchants = df['MERCHANT_NAME'].value_counts().head(5).index
                # Filter only top merchants
                filtered_trend = daily_trend[daily_trend['MERCHANT_NAME'].isin(top_merchants)]
                # Plot
                plt.figure(figsize=(14,8))
                sns.lineplot(data=filtered_trend, x='TXN_DAY', y='Transaction_Count', hue='MERCHANT_NAME')
                plt.title('Daily Transaction Trend of Top Merchants')
                plt.xlabel('Transaction Date')
                plt.ylabel('Number of Transactions')
                plt.xticks(rotation=45)
                plt.legend(title='Merchant Name')
                plt.grid(True)
                plt.show()
                st.pyplot(plt)
            
            st.subheader("🏷️ Number of Transactions by Business Type")
            # Count the business types
            business_type_counts = df['BUSINESS_TYPE'].value_counts()

            # Plotting
            fig, ax = plt.subplots(figsize=(8, 5))
            colors = ['#FF6F61', '#6B5B95', '#88B04B', '#F7CAC9', '#92A8D1']  # Customize colors
            business_type_counts.plot(kind='bar', color=colors, ax=ax)

            ax.set_title('Number of Transactions by Business Type', fontsize=16)
            ...
            # Get Top 5 Merchants by Transaction Count per Month
            top_5_per_month = (
                monthly_merchant_counts
            ...
            # Combine mobile number and name for better labeling in plot
            top_5_per_month['MERCHANT_LABEL'] = top_5_per_month['MERCHANT_NAME'] + '\n📞' + top_5_per_month['MERCHANT_MOBILE'].astype(str)

            # Plotting the Top 5 Merchants by Transaction Count per Month
            plt.figure(figsize=(14, 8))
            sns.set(style="whitegrid")

            # Convert TXN_MONTH to string for clearer x-axis labels
            top_5_per_month['TXN_MONTH'] = top_5_per_month['TXN_MONTH'].astype(str)
            ...
            num_plots = len(top_merchants)
            fig, axes = plt.subplots(num_plots, 1, figsize=(14, 8 * num_plots), sharex=True)
            
            # Iterate over each merchant and plot separately
            for i, merchant in enumerate(top_merchants):
                ax = axes[i]
                merchant_data = filtered_trend[filtered_trend['MERCHANT_NAME'] == merchant]
                sns.lineplot(data=merchant_data, x='TXN_DAY', y='Transaction_Count', ax=ax)
                ax.set_title(f'Daily Transaction Trend for {merchant}')
    ...
    with tab4:
        ...
        # reason_counts.plot(kind='bar', color='orange', ax=ax2)
        ...
        st.subheader("📉 Failure Reasons Count")
        reason_counts = failed_df['REASON'].value_counts()
        top_reasons = reason_counts[reason_counts > 10]  # show only reasons with >10 failures
        ...
        st.pyplot(fig2)
            ...
            # ---- Payment Mode Failure % (excluding BNPL) ----
        ...
        daily_counts = failed_df.groupby('DATE').size()
        fig7, ax7 = plt.subplots(figsize=(14, 5))
        sns.lineplot(x=daily_counts.index, y=daily_counts.values, marker="o", ax=ax7)
        ax7.set_title("Daily Failed Transactions")
        ax7.set_xlabel("Date")
        ax7.set_ylabel("Number of Failed Transactions")
        plt.xticks(rotation=45)
        st.pyplot(fig7)

        st.subheader("📆 Monthly Failed Transactions")
        monthly_counts = failed_df.groupby('MONTH').size()
        fig8, ax8 = plt.subplots(figsize=(10, 4))
        sns.barplot(x=monthly_counts.index, y=monthly_counts.values, palette="coolwarm", ax=ax8)
        ax8.set_title("Monthly Failed Transactions")
        ax8.set_xlabel("Month")
        ax8.set_ylabel("Count")
        st.pyplot(fig8)

        st.subheader("🕒 Hourly Failed Transactions")
        hourly_counts = failed_df.groupby('HOUR').size()
        fig9, ax9 = plt.subplots(figsize=(10, 4))
        sns.barplot(x=hourly_counts.index, y=hourly_counts.values, palette="flare", ax=ax9)
        ax9.set_title("Hourly Failed Transactions")
        ...
        merchant_failure_counts = failed_df['MERCHANT_NAME'].value_counts().head(10)
        ...
        for merchant in merchant_failure_counts.index:
            merchant_reasons = failed_df[failed_df['MERCHANT_NAME'] == merchant]['REASON'].value_counts().head(5)
